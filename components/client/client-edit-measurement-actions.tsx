...
import { Pencil, Trash2, X, AlertTriangle, Calendar } from 'lucide-react'
import { editClientMeasurementAction, deleteClientMeasurementAction } from '@/app/client/my-progress/actions'
...
export function ClientEditMeasurementActions({ m }: { m: Measurement }) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const res = await deleteClientMeasurementAction(m.id)
      ...
      <div className="modal-overlay">
        <div className="modal-dialog" style={{ maxWidth: 560 }}>
          <div className="flex items-center justify-between" style={{ marginBottom: '1.25rem' }}>
            <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700 }}>Edit My Measurement</h3>
          ...
          <form
            onSubmit={(e) => {
              ...
              setError(null)
              startTransition(async () => {
                const res = await editClientMeasurementAction(null, fd)
          ...
          >
            ...
            <div className="form-group" style={{ marginBottom: 'var(--space-3)' }}>
              <label htmlFor={`edit-client-date-${m.id}`} style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 700, color: 'var(--brand-700)' }}>
                <Calendar size={14} /> Measurement Date
              </label>
              <input
                id={`edit-client-date-${m.id}`}
                name="measuredAt"
                type="date"
